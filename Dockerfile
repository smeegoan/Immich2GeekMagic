...
RUN echo "0 1 * * * root cd /app && python3 immich_to_geekmagic.py >> /var/log/cron.log 2>&1" > /etc/cron.d/immich-sync && \
...
RUN echo '#!/bin/bash' > /start.sh && \
    ...
    echo 'cd /app && python3 immich_to_geekmagic.py' >> /start.sh && \
